...
Release configuration and fails if it finds either. Until `modelpipe-ffi`
lands, everything below describes behaviour that a shipped build does not
have — `UnavailablePipeConnector` refuses, and refusing is all it does.
